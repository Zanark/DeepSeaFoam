...
Scenery belongs **only to the showcase**, not the application exports. The site now includes the owner's supplied **2:56 background music** as a **2,817,068-byte MP3** rather than the 33.8 MB WAV. **Play music** is explicit opt-in: no autoplay, no audio request before the first press, and no remembered permission on reload. Playback loops; **Pause music** preserves position. Leaving the tab or page pauses it, and returning never resumes it automatically. It starts at 35% element volume where supported; phone hardware volume remains authoritative.

**Pause motion** and reduced-motion preferences affect scenery, not manually requested music. The player exposes loading/cancellation, retryable errors and a live status message; without JavaScript, a direct MP3 link replaces its controls. The music is website-only, and its original artist/license were not supplied; it is excluded from the original theme/code MIT grant. [Media provenance](docs/showcase-artwork.json) and the [artwork/audio notice](site/artwork-NOTICE.txt) record the conversion and rights boundary.

The **3 MiB all-file cap counts the music and every other deployed file**, with the earlier **256 KiB cap retained for non-audio assets**. This explicit increase accommodates the requested soundtrack without making the initial page download it. There is no video, WebGL, external image service, or runtime library dependency. The photographic inspiration remains an external link. **Pause motion** freezes scenery and clears transient wakes; reduced motion skips the descent. **Skip descent**, Escape, Tab, navigation, or scrolling also bypasses the opening. Without JavaScript, the page and native creature reveals remain usable; the dynamic palette instead links to the README swatches.
